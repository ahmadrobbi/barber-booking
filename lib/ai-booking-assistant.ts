...
export async function generateAiFaqReply(params: {
  ...
  try {
    ...
    const jsonString = answerContent ? extractJsonObject(answerContent) : null;
    const parsedReply = jsonString ? AssistantResponseSchema.safeParse(JSON.parse(jsonString)) : null;
    const reply = parsedReply?.success && parsedReply.data.reply.trim()
      ? parsedReply.data.reply.trim()
      : deterministicReply;
    ...
    await recordChatbotAiEvent({
      ...
      route: "faq",
      intent: "faq",
      confidence: parsedReply?.success ? parsedReply.data.confidence : retrieval.matches[0]?.score ?? 0.5,
      model: getAiConfig().model,
      knowledgeHitCount: retrieval.matches.length,
      retrievalMs: retrieval.retrievalMs,
      aiMs,
      totalMs: Date.now() - startedAt,
      fallbackUsed: !parsedReply?.success || !answerContent,
      error: parsedReply?.success ? null : "FAQ AI response was not valid JSON",
      metadata: {
        toolName,
        messagePreview: params.message.slice(0, 120),
